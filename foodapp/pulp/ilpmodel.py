...
def print_status():
    '''
    No parameters for the moment. Maybe a user_id after.
    '''
    return "Status:", LpStatus[prob.status]

def print_opt_recipes():
    '''
    ...
    #selected_recipes = Recipe.objects.filter(id__in=list_selected_rid).values('rid','recipe_name')
    return list_selected_rid

def print_opt_ingredients():
    '''
    Returns a dictionary of the form {Ingredient : Number of packages to buy} of selected ingredients
    '''
